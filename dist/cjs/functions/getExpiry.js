...
const raw = async (dnsArgs, name, { contract } = {}) => {
  const labels = name.split(".");
  const contractToUse = getContractToUse(contract, labels);
  return contractToUse === "nameWrapper" ? getWrapperExpiry(dnsArgs, labels) : getRegistrarExpiry(dnsArgs, labels);
};
...
const decode = async (dnsArgs, data, name, { contract } = {}) => {
  if (data === null) {
    return {
      expiry: null,
      gracePeriod: null
  ...
  const labels = name.split(".");
  const contractToUse = getContractToUse(contract, labels);
  return contractToUse === "nameWrapper" ? decodeWrapperExpiry(dnsArgs, data) : decodeRegistrarExpiry(dnsArgs, data);
};
